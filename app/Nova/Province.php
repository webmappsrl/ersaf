...
class Province extends Resource
{
    ...
    public function cards(NovaRequest $request)
    {
        if (! is_null($request['resourceId'])) {
            ...
            $data = DB::table('ec_track_province')
                ->join('ec_tracks', 'ec_track_province.ec_track_id', '=', 'ec_tracks.id')
                ->where('ec_track_province.province_id', $request['resourceId'])
                ->select('ec_tracks.osm2cai_status', DB::raw('COUNT(*) as total'))
                ->groupBy('ec_tracks.osm2cai_status')
                ->get()
