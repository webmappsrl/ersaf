...
class Region extends Model implements OsmfeaturesSyncableInterface
{
    ...
    protected $fillable = ['osmfeatures_id', 'osmfeatures_data', 'osmfeatures_updated_at', 'geometry', 'name', 'num_expected', 'ec_tracks_intersecting', 'code'];
    ...
    {
        return $this->belongsToMany(HikingRoute::class, 'ec_track_region', 'region_id', 'ec_track_id');
    }
    ...
    public function getGeojsonComplete(): string
    {
        ...
        $hikingRoutes = $this->hikingRoutes() // Use the relationship query builder
            ...
            ->selectRaw('ec_tracks.*, ST_AsGeoJSON(geometry) as geom_geojson') // Select all HR fields + geometry
