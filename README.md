...
ersaf is a Laravel 11 project based on php 8.2 and Laravel Nova 4 with postgis extension.
...
```sh
git clone git@github.com:webmappsrl/ersaf.git ersaf
git flow init
```
...
docker exec -u 0 -it php81-ersaf bash
chown -R 33 .
```
...
AWS_BUCKET=ersaf-bucket
AWS_ENDPOINT=http://minio-ersaf:9003
AWS_URL=http://localhost:9002
AWS_USE_PATH_STYLE_ENDPOINT=true

# Email Development  
MAIL_MAILER=smtp
MAIL_HOST=mailpit-ersaf
MAIL_PORT=1025
```
...
```bash
docker exec php81-ersaf php artisan config:clear
docker exec php81-ersaf php artisan config:cache
```

E riavvia Horizon per applicare le nuove configurazioni:

```bash
docker exec php81-ersaf php artisan horizon:terminate
docker exec -d php81-ersaf php artisan horizon
```
...
```bash
docker exec php81-ersaf php artisan tinker --execute="try { DB::connection('geohub')->getPdo(); echo 'Connessione geohub OK'; } catch(Exception \$e) { echo 'Errore connessione: ' . \$e->getMessage(); }"
```
