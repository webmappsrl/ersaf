...
class CaiHut extends Model implements OsmfeaturesSyncableInterface
{
    ...
    protected static function booted()
    {
        static::saved(function ($caiHut) {
            if ($caiHut->isDirty('geometry')) {
                CheckNearbyHikingRoutesJob::dispatch($caiHut, config('osm2cai.ec_track_buffer'))->onQueue('geometric-computations');
            }
        });
    }
    ...
    {
        return $this->belongsToMany(HikingRoute::class, 'ec_track_cai_hut')->withPivot(['buffer']);
    }
