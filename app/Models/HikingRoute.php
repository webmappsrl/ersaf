...
class HikingRoute extends EcTrack
{
    ...
    public function getDistanceCompAttribute(): ?float
    {
        ...
        $result = DB::selectOne(
            'SELECT ST_Length(geometry, true) AS distance FROM ec_tracks WHERE id = ?',
            [$this->id]
        );
    ...
    {
        return $this->belongsToMany(Region::class, 'ec_track_region', 'ec_track_id', 'region_id');
    }

    public function provinces()
    {
        return $this->belongsToMany(Province::class, 'ec_track_province', 'ec_track_id', 'province_id');
    }

    public function clubs()
    {
        return $this->belongsToMany(Club::class, 'ec_track_club', 'ec_track_id', 'club_id');
    }

    public function areas()
    {
        return $this->belongsToMany(Area::class, 'area_ec_track', 'ec_track_id', 'area_id');
    }

    public function sectors()
    {
        return $this->belongsToMany(Sector::class, 'ec_track_sector', 'ec_track_id', 'sector_id')->withPivot(['percentage']);
    }
    ...
    {
        return $this->belongsToMany(Itinerary::class, 'ec_track_itinerary', 'ec_track_id', 'itinerary_id');
    }

    public function nearbyCaiHuts()
    {
        return $this->belongsToMany(CaiHut::class, 'ec_track_cai_hut')->withPivot(['buffer']);
    }

    public function nearbyNaturalSprings()
    {
        return $this->belongsToMany(NaturalSpring::class, 'ec_track_natural_spring')->withPivot(['buffer']);
    }

    public function nearbyEcPois()
    {
        return $this->belongsToMany(EcPoi::class, 'ec_track_ec_poi')->withPivot(['buffer']);
    }

    /**
     * Override della relazione ecPois del parent EcTrack per usare la tabella corretta
     * La tabella pivot per HikingRoute è 'ec_track_ec_poi' non 'ec_poi_hiking_route'
     */
    public function ecPois(): BelongsToMany
    {
        return $this->belongsToMany(EcPoi::class, 'ec_track_ec_poi')->withPivot('order')->orderByPivot('order');
    }

    public function mountainGroups()
    {
        return $this->belongsToMany(MountainGroups::class, 'mountain_group_ec_track', 'ec_track_id', 'mountain_group_id');
    }
...
SQL;
    ...
    {
        $buffer = config('osm2cai.ec_track_buffer');
    ...
    public function getPolesWithBuffer(float $bufferDistance = 10)
    {
        ...
        if (! empty($this->geometry_raw_data)) {
            $mergedGeojson = DB::table('ec_tracks')
                ->where('id', $this->id)
