...
class ImportElementFromOsm2caiJob implements ShouldQueue
{
    ...
    private function recalculateEdges($edges)
    {
        ...
        $legacyHr = DB::connection('legacyosm2cai')->table('ec_tracks')
    ...
    {
        $pivotTable = DB::connection('legacyosm2cai')->table('ec_track_itinerary')
            ->where('itinerary_id', $itineraryId)->get();
        $hrIds = $pivotTable->pluck('ec_track_id')->toArray();
        $legacyHr = DB::connection('legacyosm2cai')->table('ec_tracks')
            ->whereIn('id', $hrIds)
            ->get('relation_id')
            ->toArray();
