...
return new class extends Migration
{
    ...
    public function up(): void
    {
        DB::transaction(function () {
            ...
            $hasGeometryOsm = Schema::hasColumn('ec_tracks', 'geometry_osm');

            if ($hasGeometryOsm) {
                // Check if there are records with geometry_osm data
                $hasData = DB::table('ec_tracks')->whereNotNull('geometry_osm')->exists();
                ...
                if ($hasData) {
                    ...
                    DB::statement('UPDATE ec_tracks SET geometry_osm_temp = ST_Force3D(geometry_osm) WHERE geometry_osm IS NOT NULL');
            ...
            $hasGeometryRawData = Schema::hasColumn('ec_tracks', 'geometry_raw_data');

            if ($hasGeometryRawData) {
                // Check if there are records with geometry_raw_data data
                $hasData = DB::table('ec_tracks')->whereNotNull('geometry_raw_data')->exists();
                ...
                if ($hasData) {
                    ...
                    // 2. Update the temporary column with 3D geometries from the old column
                    DB::statement('UPDATE ec_tracks SET geometry_raw_data_temp = ST_Force3D(geometry_raw_data) WHERE geometry_raw_data IS NOT NULL');
        ...
        DB::transaction(function () {
            ...
            if ($hasGeometryOsm) {
                ...
                if ($hasData) {
                    ...
                    DB::statement('UPDATE ec_tracks SET geometry_osm_temp = ST_Force2D(geometry_osm) WHERE geometry_osm IS NOT NULL');
            ...
            if ($hasGeometryRawData) {
                ...
                if ($hasData) {
                    ...
                    // 2. Update the temporary column with 2D geometries from the old column
                    DB::statement('UPDATE ec_tracks SET geometry_raw_data_temp = ST_Force2D(geometry_raw_data) WHERE geometry_raw_data IS NOT NULL');
