    ...
    public function cards(Request $request)
    {
        if (! is_null($request['resourceId'])) {
            ...
            $numbers = Cache::remember('sector_' . $request['resourceId'] . '_numbers', 60, function () use ($request) {
                return DB::table('ec_track_sector')
                    ->join('ec_tracks', 'ec_track_sector.ec_track_id', '=', 'ec_tracks.id')
                    ->where('ec_track_sector.sector_id', $request['resourceId'])
                    ->selectRaw('
                    COUNT(CASE WHEN ec_tracks.osm2cai_status = 1 THEN 1 END) as tot1,
                    COUNT(CASE WHEN ec_tracks.osm2cai_status = 2 THEN 1 END) as tot2, 
                    COUNT(CASE WHEN ec_tracks.osm2cai_status = 3 THEN 1 END) as tot3,
                    COUNT(CASE WHEN ec_tracks.osm2cai_status = 4 THEN 1 END) as tot4
                ')
                    ->first();
            });
