...
class DashboardCardsHelper
{
        ...
        $regions = Cache::remember('percorsi-favoriti-dashboard-data', 60 * 60 * 24 * 2, function () {
            return DB::table('regions')
                ->select([
                    ...
                    DB::raw('(SELECT COUNT(*) FROM ec_track_region hrr JOIN ec_tracks hr ON hrr.ec_track_id = hr.id WHERE hrr.region_id = regions.id AND hr.region_favorite = true) as favorite_routes_count'),
                    DB::raw('(SELECT COUNT(*) FROM ec_track_region hrr JOIN ec_tracks hr ON hrr.ec_track_id = hr.id WHERE hrr.region_id = regions.id AND hr.osm2cai_status = 4) as sda4_routes_count'),
                ])
    ...
    public function getUtentiDashboardCards()
    {
        ...
        $mostActiveUsers = Cache::remember('mostActiveUsers', 60 * 60 * 24 * 2, function () {
            return DB::select("
                ...
                FROM users u
                JOIN ec_tracks hr ON u.id = hr.validator_id
    ...
    public function getSALMiturAbruzzoDashboardCards()
    {
        ...
        $totalsGlobal = Cache::remember('sal_mitur_totals', 60 * 60 * 24 * 2, function () {
            ...
            $sumHikingRoutes = DB::select('SELECT count(*) as count FROM ec_tracks WHERE osm2cai_status = 4')[0]->count;
        ...
        $items = Cache::remember('sectors_dashboard_items', now()->addDays(2), function () use ($sectorsIds) {
            return DB::table('sectors')
                ->select(
                    ...
                    DB::raw('COUNT(CASE WHEN ec_tracks.osm2cai_status = 1 THEN 1 END) as tot1'),
                    DB::raw('COUNT(CASE WHEN ec_tracks.osm2cai_status = 2 THEN 1 END) as tot2'),
                    DB::raw('COUNT(CASE WHEN ec_tracks.osm2cai_status = 3 THEN 1 END) as tot3'),
                    DB::raw('COUNT(CASE WHEN ec_tracks.osm2cai_status = 4 THEN 1 END) as tot4')
                )
                ->leftJoin('ec_track_sector', 'ec_track_sector.sector_id', '=', 'sectors.id')
                ->leftJoin('ec_tracks', 'ec_tracks.id', '=', 'ec_track_sector.ec_track_id')
...
<?php
    ...
    private function _getSectorsTableCardByModelClassName($user, $modelClassName): CustomTableCard
    {
        ...
        $items = DB::table('sectors')
            ...
            ->leftJoin('ec_track_sector', 'sectors.id', '=', 'ec_track_sector.sector_id')
            ->leftJoin('ec_tracks', 'ec_track_sector.ec_track_id', '=', 'ec_tracks.id')
            ->whereIn('sectors.id', $sectorsIds)
            ->groupBy('sectors.id', 'sectors.full_code', 'sectors.num_expected')
            ->get();
