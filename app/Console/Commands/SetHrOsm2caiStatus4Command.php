...
class SetHrOsm2caiStatus4Command extends Command
{
    ...
    public function handle()
    {
        ...
        $legacyHikingRoutes = $legacyConnection->table('ec_tracks')->select('relation_id')->where('osm2cai_status', 4)->get();
        ...
        if (! empty($notFoundHikingRoutes)) {
            file_put_contents(
                storage_path('not_found_ec_tracks_status_4.txt'),
                implode("\n", $notFoundHikingRoutes) . "\n"
            );
        }
        ...
        if (! empty($notFoundHikingRoutes)) {
            $this->info('List of not found hiking routes written to: ' . storage_path('not_found_ec_tracks_status_4.txt'));
        }
